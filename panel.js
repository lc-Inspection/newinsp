...
// İkinci Inspection verisi de AYNI sebepten (bkz. yukarıdaki not) dosyanın en
// başında tanımlanır — kullanıcı talebiyle: Dashboard'ın İLK boyaması
// (renderDashboard -> getEfektifPerfSeviye) artık Teknik Skor VE İkinci
// Insp'i de kullandığı için, bu veri de cache'ten SENKRON ve ERKEN
// yüklenmezse önce eksik veriyle (sadece Günlük Ort'a göre) bir sonuç,
// birkaç saniye sonra ağdan veri gelince FARKLI bir sonuç gösterilip
// rakamlar "zıplıyordu". Aşağıdaki IKINCI_INSPECTION_LS_KEY ile aynı.
const IKINCI_INSPECTION_LS_KEY = 'lc_ikinci_inspection_cache';
...
try { if (typeof loadIkinciInspectionFromLocalStorage === 'function') loadIkinciInspectionFromLocalStorage(); } catch(e) {}
...
// (GÜNCELLEME — kullanıcı talebiyle) Bir inspector'ın bir seviyeye (ör.
// Fark Yaratan) ulaşabilmesi için elindeki TÜM metriklerin (veri
// olanların) bu tablodaki eşiği sağlaması gerekir — tek bir metrik bile
// altında kalırsa genel seviye o metriğin seviyesine düşer ("en zayıf
// halka" kuralı; artık aritmetik ortalama KULLANILMIYOR). Bkz. _ceyrekGenelSeviye().
...
// NOT: ikinciInspectionData artık dosyanın EN BAŞINDA tanımlı (bkz. TI_SKOR_LS_KEY
// yanındaki not) — erken cache yüklemesi TDZ hatası vermesin diye.
...
async function pushPerformansManual(ev) {
  ...
  try {
    ...
    const performansDataTemiz = performansData.map(inspector => {
      ...
      // Google Sheets aynasındaki (mirror) "Teknik İnceleme Skoru (%)"
      // sütunu için: DÜZELTME (kullanıcı talebiyle) — bu değer eskiden
      // sunucu tarafında (PHP backend) AYRI bir sorguyla dolduruluyordu ve
      // panel.js'in Dashboard kartında CANLI gösterdiği değerle (o an
      // içinde bulunulan çeyreğe göre filtrelenmiş) tutarsız çıkabiliyordu
      // (ör. bir inspector Dashboard'da "Teknik Skor yok" gösterirken,
      // Sheets'teki mirror'da eski/farklı bir sayı görünüyordu). Artık bu
      // değer de BURADA, panel.js'in TEK doğru kaynağı olan
      // getTeknikIncelemeSkorForInspector() ile hesaplanıp doğrudan
      // gönderiliyor — böylece Dashboard kartı, Çeyrek Performans Arşivi
      // VE Sheets mirror'ı HER ZAMAN aynı sayıyı gösterir.
      const _tiMirror = getTeknikIncelemeSkorForInspector(inspector.ins);
      const teknikSkoruMirror = (_tiMirror && _tiMirror.count > 0) ? _tiMirror.percent : null;
...
// sonra genel seviye bu puanların ORTALAMASI DEĞİL, veri bulunan
// metrikler arasındaki EN DÜŞÜK (en kötü) seviyedir — bkz.
// _ceyrekGenelSeviye(). Yani bir inspector bir seviyeye ulaşmak için
// TÜM metriklerde o seviyenin eşiğini sağlamalıdır. Veri olmayan ("—")
// metrikler değerlendirmeye katılmaz; Günlük Ort. her zaman hesaplanabilir
...
// kategoriye gireceği artık TEK bir metriğe değil üçünün en zayıfına göre
// belirleniyor.
...
function _ceyrekMetrikHucre(veri) {
  ...
  const pTitle = pSeviye ? ` title="${pSeviye.detay || ''} · En düşük metrik: ${pSeviye.ortalamaPuan} puan (genel seviyeyi bu belirler)"` : '';
  ...
  const pPuanEtiket = pSeviye ? ` <span style="font-size:11px;color:var(--muted2);font-weight:400">(${pSeviye.ortalamaPuan}/5)</span>` : '';
  return `
...
// Zayıf=1 puan. Genel Performans, veri bulunan metrikler arasındaki EN
// DÜŞÜK (en kötü) seviyedir — bir inspector bir seviyeye ulaşmak için
// TÜM metriklerde o seviyenin eşiğini sağlamalıdır (bkz. _ceyrekGenelSeviye()).
...
function _ceyrekMetrikSeviye(deger, metrikKey) {
  ...
  if (!esik) return null;
  // NOT (düzeltildi): önceden burada "> esik.farkYaratan" (KESİNLİKLE BÜYÜK)
  // kontrolü vardı — yani tabloda "450" yazsa bile tam 450'ye sahip biri
  // Fark Yaratan SAYILMIYOR, bir alt seviyeye (İyi) düşüyordu. Kullanıcının
  // "450/95/95 OLMALI" ifadesiyle (yani bu değere ULAŞMAK yeterli olmalı)
  // çelişiyordu — ">=" yapıldı, artık tabloda yazan değerin TAM KARŞILIĞI
  // da o seviyeyi hak ediyor.
...
// (GÜNCELLEME — kullanıcı talebiyle: ORTALAMA yerine "EN ZAYIF HALKA" kuralı)
...
// puan) çevrilir. Genel seviye artık bu puanların ortalaması DEĞİL, VERİSİ
// OLAN metrikler arasındaki EN DÜŞÜK (en kötü) seviyedir — yani bir
// inspector bir seviyeye (ör. Fark Yaratan) ulaşabilmek için elindeki
// TÜM metriklerde o seviyenin eşiğini sağlamalıdır; tek bir metrik bile
// altında kalırsa genel seviye o metriğin seviyesine düşer.
// Örnek: Günlük Ort. Fark Yaratan (5) + Teknik Skor İyi (4) + İkinci Insp
// Orta (3) → en düşüğü Orta (3) → genel seviye Orta. Veri olmayan ("—")
// metrikler değerlendirmeye katılmaz.
function _ceyrekGenelSeviye(veri) {
  ...
  // En düşük puanlı (en zayıf) adayı bul — genel seviye budur.
  const enZayif = adaylar.reduce((min, a) => a.seviye.sira < min.seviye.sira ? a : min, adaylar[0]);
  const sonuc = enZayif.seviye;
  const enDusukPuan = sonuc.sira;
  ...
  return { ...sonuc, ortalamaPuan: enDusukPuan, detay };
}
...
async function fetchIkinciInspectionData() {
  ...
  try {
    ...
    if (data?.status === 'ok' && Array.isArray(data.kayitlar)) {
      ...
      saveIkinciInspectionToLocalStorage();
    }
  } catch(e) {
...
// İkinci Inspection kayıtlarını localStorage'dan geri yükler (kullanıcı
// talebiyle eklendi) — bu, sayfa açılışında Dashboard'ın İLK render'ının
// bile Teknik Skor + İkinci Insp verisiyle birlikte, EKSİKSİZ hesaplanmış
// sayılarla çizilmesini sağlar; ağdan taze veri gelince rakamlar "zıplamaz".
function saveIkinciInspectionToLocalStorage() {
  try { localStorage.setItem(IKINCI_INSPECTION_LS_KEY, JSON.stringify(ikinciInspectionData)); } catch(e) { console.warn('İkinci Inspection cache yazma hatası:', e); }
}
function loadIkinciInspectionFromLocalStorage() {
  try {
    const raw = localStorage.getItem(IKINCI_INSPECTION_LS_KEY);
    if (raw) {
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed)) ikinciInspectionData = parsed;
    }
  } catch(e) { console.warn('İkinci Inspection cache okuma hatası:', e); }
}
